...
import { LanguageTestsStep } from "@/components/onboarding/language-tests-step"
...
export default function OnboardingPage() {
  ...
  const isStepValid = () => {
    switch (currentStep) {
      ...
      case 4:
        return data.desiredCountries.length > 0 && data.desiredDegree && data.scholarshipAmount
      case 5:
        return true
      default:
        return false
    ...
    <div className="min-h-screen bg-background py-8">
      <div className="container mx-auto max-w-4xl px-4">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Let's set up your profile to find the perfect scholarships for you</p>
        </div>
          ...
          <div className="flex items-center space-x-4 overflow-x-auto pb-2">
            {steps.map((step, index) => (
              <div key={step.id} className="flex items-center">
                ...
                {index < steps.length - 1 && <div className="w-12 h-px bg-muted-foreground/30 mx-4 mt-[-20px]" />}
              </div>
            ))}
          </div>
        </div>
