...
const filterOptions = {
  ...
  country: ['United States', 'United Kingdom', 'Canada', 'Australia', 'Germany', 'Netherlands', 'Sweden', 'Norway', 'Denmark', 'Switzerland', 'France', 'Japan', 'South Korea', 'Singapore', 'Other'],
  ...
  eligibleFields: ['Computer Scien', 'Data Science', 'Engineering', 'Business Admin', 'Economics', 'Medicine', 'Law', 'Psychology', 'Biology', 'Chemistry', 'Physics', 'Mathematics', 'Other'],
...
const ScholarshipFilter = () => {
  ...
  const handleSelect = (category: keyof FiltersState, value: string | number, isMultiSelect: boolean = false) => {
    setFilters(prev => {
  ...
  const renderOptions = (
    ...
    <div className="mb-6">
      <h3 className="font-semibold text-gray-800 mb-3 capitalize">{category.replace(/([A-Z])/g, ' $1')}</h3>
          ...
          >
            ...
            <div className="flex justify-between items-center border-b p-4 sm:p-6 sticky top-0 bg-white z-10">
              <div className="flex items-center gap-3">
                <FilterIcon />
                <h2 className="text-xl font-bold text-gray-800">Bộ lọc tìm kiếm học bổng</h2>
              </div>
              <button 
            ...
            <div className="overflow-y-auto p-4 sm:p-6">
              ...
              {renderOptions('applicationMonth', filterOptions.applicationMonth, true)}
              {renderOptions('eligibility', filterOptions.eligibility)}
              {renderOptions('eligibleFields', filterOptions.eligibleFields, true)}
              {renderOptions('minGpa', filterOptions.minGpa)}

              {/* Trường đặc biệt: Language Certificate */}
              <div className="mb-6">
                  <h3 className="font-semibold text-gray-800 mb-3">Language Certificate</h3>
                  <div className="flex flex-col items-start gap-4">
                      {filterOptions.languageCertificate.map(option => {
                          const isSelected = filters.languageCertificate.hasOwnProperty(option);
                          return (
                            <div key={option} className="flex items-center gap-4 w-full" style={{maxWidth: '450px'}}>
                              <button
                              ...
                              >
                                {isSelected ? <CheckIcon /> : <span className="w-4 mr-1"></span>}
                              ...
                              <div className="relative flex-grow h-12">
                                {isSelected && (
                                    <input
                                        ...
                                        onChange={(e) => handleLanguageScoreChange(option, e.target.value)}
