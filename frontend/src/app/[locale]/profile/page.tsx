...
const mapProfileToPersonalInfo = (
	...
	base.nationality =
		firstString(profile?.desired_countries) ?? base.nationality;
...
export default function ProfilePage() {
	...
	const mergePersonalInfo = useCallback(
		(profileData: UserProfile | null) => {
			setPersonalInfo((prev) => {
				...
				return mapProfileToPersonalInfo(
					profileData,
					fallback,
					user?.email ?? null,
					user?.name ?? null,
	...
	const buildUpdatePayload = (): UpdateProfilePayload => {
		const payload: UpdateProfilePayload = {
			...
			birth_date: personalInfo.dateOfBirth
				...
				: undefined,
			desired_countries: personalInfo.nationality
	...
	const summary: UserProfileSummary = useMemo(() => {
		...
		const email =
			...
			"";
		const nationality =
			personalInfo.nationality ?? firstString(profile?.desired_countries) ?? "";
		const level = personalInfo.currentLevel ?? asString(profile?.degree) ?? "";
		const major = asString(profile?.field_of_study) ?? "";
		const gpa =
			typeof profile?.gpa_range_4 === "number" ? profile.gpa_range_4 : null;
						...
						<div className="flex flex-col items-start justify-between space-y-4 md:flex-row md:items-center md:space-y-0">
							<div className="flex items-center space-x-4">
								<Avatar className="h-20 w-20">
									<AvatarImage
										src={summary.avatar || "/placeholder.svg"}
										alt={summary.name}
									/>
								...
								<div>
									...
									<div className="mt-2 flex items-center space-x-2">
										...
										{summary.major ? (
											<Badge variant="outline">{summary.major}</Badge>
										) : null}
										{summary.nationality ? (
											<Badge variant="outline">{summary.nationality}</Badge>
										) : null}
									</div>
								</div>
							</div>
