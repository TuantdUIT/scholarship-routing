...
export class ScholarshipApi {
   ...
   */
  static async filterScholarships(params: ScholarshipFilterParams): Promise<ScholarshipSearchResponse> {
   ...
   */
  static async getLatestScholarships(size = 10, offset = 0): Promise<ScholarshipSearchResponse> {
  ...
  static transformResponseToUI(response: ScholarshipSearchResponse): {
    ...
    return {
      ...
      scholarships: response.items.map(item => this.transformToUIFormat(item)),
    };
  }
} 