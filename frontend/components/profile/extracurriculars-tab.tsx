...
interface Activity {
  ...
  organization: string
  role: string
  type: "volunteer" | "leadership" | "competition" | "club" | "sports" | "arts" | "community"
  startDate: string
  endDate: string
  current: boolean
  description: string
  ...
  const [activities, setActivities] = useState<Activity[]>([
    {
      ...
      achievements: ["Tutored 25+ students", "Improved average test scores by 30%", "Organized study groups"],
    },
    {
      id: "2",
      ...
      endDate: "",
      current: true,
      description: "Leading student initiatives and representing student interests in university policy discussions.",
      achievements: ["Led 3 major campus initiatives", "Increased student engagement by 40%"],
    },
  ])
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Your leadership, volunteer work, and other activities</p>
        </div>
        {isEditMode && (
            ...
            <CardHeader className="pb-3">
              <div className="flex justify-between items-start">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <CardTitle className="text-base">{activity.title || "New Activity"}</CardTitle>
                    <p className="text-sm text-muted-foreground">{activity.organization}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <Badge variant={getTypeColor(activity.type)}>{activity.type}</Badge>
                  {isEditMode && (
                    <Button
                      variant="ghost"
                      size="sm"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  {isEditMode ? (
                    ...
                    <div className="p-2 text-sm capitalize">{activity.type}</div>
                  )}
                </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <label className="flex items-center space-x-2 text-sm">
                        <input
                          ...
                          onChange={(e) => updateActivity(activity.id, "current", e.target.checked)}
                        />
        ...
        <Card className="text-center py-8">
          <CardContent>
            ...
            <p className="text-muted-foreground mb-4">
              Add your extracurricular activities to showcase your leadership and community involvement
            </p>
            {isEditMode && <Button onClick={addActivity}>Add Your First Activity</Button>}
          </CardContent>
        </Card>
      )}
    </div>
