...
export function PublicationsTab({ isEditMode }: PublicationsTabProps) {
  const [publications, setPublications] = useState<Publication[]>([
    {
      id: "1",
      title: "Deep Learning Approaches for Natural Language Processing in Educational Applications",
  ...
  const updatePublication = (id: string, field: keyof Publication, value: string) => {
    setPublications(publications.map((pub) => (pub.id === id ? { ...pub, [field]: value } : pub)))
  }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Your academic publications and research work</p>
        </div>
        {isEditMode && (
          <Button onClick={addPublication} size="sm" className="flex items-center">
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-start space-x-2 flex-1">
                  ...
                  <div className="flex-1">
                    <CardTitle className="text-base leading-tight">{publication.title || "New Publication"}</CardTitle>
                    <p className="text-sm text-muted-foreground mt-1">{publication.authors}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-2 ml-4">
                  <Badge variant={getTypeColor(publication.type)}>{publication.type}</Badge>
                  <Badge variant={getStatusColor(publication.status)}>{publication.status.replace("-", " ")}</Badge>
                  {isEditMode && (
                    <Button
                      variant="ghost"
                      size="sm"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  {isEditMode ? (
                    ...
                    <div className="p-2 text-sm capitalize">{publication.type}</div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {isEditMode ? (
                    <select
                      ...
                      onChange={(e) => updatePublication(publication.id, "status", e.target.value)}
                      className="w-full p-2 border rounded-md"
                    >
                    ...
                    <div className="p-2 text-sm capitalize">{publication.status.replace("-", " ")}</div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {isEditMode ? (
                    ...
                    <div className="p-2 text-sm">{publication.doi || "Not available"}</div>
                  )}
                </div>
        ...
        <Card className="text-center py-8">
          <CardContent>
            ...
            <p className="text-muted-foreground mb-4">
              Add your research publications to strengthen your scholarship applications
            </p>
            {isEditMode && <Button onClick={addPublication}>Add Your First Publication</Button>}
          </CardContent>
        </Card>
      )}
    </div>
