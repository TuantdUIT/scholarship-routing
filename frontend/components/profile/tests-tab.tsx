  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Your standardized test results for scholarship applications</p>
      </div>
        ...
        {testScores.map((test, index) => (
          <Card key={test.type}>
            <CardHeader className="pb-3">
              ...
              {test.type === "TOEFL iBT" && <CardDescription>Test of English as a Foreign Language</CardDescription>}
              {test.type === "GRE" && <CardDescription>Graduate Record Examination</CardDescription>}
              {test.type === "GMAT" && <CardDescription>Graduate Management Admission Test</CardDescription>}
            </CardHeader>
              ...
              <div className="space-y-2">
                ...
                {isEditMode ? (
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {test.score && <span className="text-sm text-muted-foreground">/ {test.maxScore}</span>}
                  </div>
                )}
              </div>
              ...
              {test.score && (
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span>{Math.round(getScorePercentage(test.score, test.maxScore))}%</span>
                  </div>
                  <Progress value={getScorePercentage(test.score, test.maxScore)} className="h-2" />
                </div>
              )}
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="text-sm text-blue-800">
              ...
              <ul className="space-y-1 text-xs">
                <li>• IELTS/TOEFL scores are required for most international scholarships</li>
                <li>• GRE is often required for graduate programs in STEM fields</li>
                <li>• GMAT is typically required for business school applications</li>
                <li>• Higher scores increase your scholarship matching opportunities</li>
              </ul>
            </div>
          </div>
        </CardContent>
