...
import { Upload, Download, FileText, CheckCircle, Clock, AlertTriangle, Eye } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function ApplicationDocuments({ application }: ApplicationDocumentsProps) {
  ...
  const getDocumentStatusBadge = (status: string) => {
    switch (status) {
      case "uploaded":
        return <Badge className="bg-green-100 text-green-800 border-0">Uploaded</Badge>
      case "pending":
        return <Badge className="bg-orange-100 text-orange-800 border-0">Pending</Badge>
      case "required":
        return <Badge className="bg-red-100 text-red-800 border-0">Required</Badge>
      default:
  ...
  const uploadedCount = application.documents.required.filter((doc: any) => doc.status === "uploaded").length
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {document.status === "pending" && (
                      <div className="text-sm text-muted-foreground">Required document - not yet uploaded</div>
                    )}
                  </div>
                </div>
                    ...
                    <Dialog>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Select the file you want to upload for this document requirement.
                          </DialogDescription>
                        </DialogHeader>
                        <div className="space-y-4">
                          ...
                          {uploadProgress[document.name] !== undefined && (
                            <div className="space-y-2">
                              ...
                              <Progress value={uploadProgress[document.name]} className="h-2" />
                            </div>
                          )}
                        </div>
                      </DialogContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2 text-sm text-muted-foreground">
            ...
            <p>• Ensure all documents are clearly readable and properly formatted</p>
            <p>• Official transcripts should be sealed or certified copies</p>
            <p>• Reference letters must be on official letterhead</p>
            <p>• Personal statements should follow the specified word count limits</p>
          </div>
        </CardContent>
      </Card>
    </div>
