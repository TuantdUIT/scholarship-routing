...
export function ApplicationTable({ applications, onStatusChange }: ApplicationTableProps) {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return statusOption ? (
      <Badge className={`${statusOption.color} border-0`}>{statusOption.label}</Badge>
    ) : (
            ...
            {applications.map((application) => {
              ...
              return (
                <TableRow key={application.id} className="hover:bg-muted/50">
                  <TableCell>
                    <div>
                      <div className="font-medium text-sm">{application.scholarshipTitle}</div>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-sm">
                        <div className={`${isUrgent ? "text-orange-600" : isExpired ? "text-red-600" : ""}`}>
                      ...
                      {isExpired && <AlertTriangle className="h-4 w-4 text-red-600" />}
                    </div>
                  </TableCell>

                  <TableCell>
                    <div className="text-sm text-muted-foreground max-w-32 truncate">{application.amount}</div>
                  </TableCell>
