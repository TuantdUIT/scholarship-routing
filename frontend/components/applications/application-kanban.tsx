...
import { Calendar, FileText, AlertTriangle, MoreHorizontal, Upload, Edit, Bell, ExternalLink } from "lucide-react"
      ...
      >
        <CardHeader className="pb-3">
          <div className="flex justify-between items-start gap-2">
            <div className="flex-1">
              <CardTitle className="text-sm font-medium leading-tight mb-1">{application.scholarshipTitle}</CardTitle>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className={`${isUrgent ? "text-orange-600" : isExpired ? "text-red-600" : "text-muted-foreground"}`}>
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-muted-foreground">
              {application.documents.uploaded.length}/{application.documents.required.length}
            </span>
          </div>

          {/* Amount */}
          <div className="text-xs text-muted-foreground">{application.amount}</div>
          ...
          {application.reminders.length > 0 && (
            <div className="flex items-center gap-1 text-xs text-orange-600">
              ...
              <span>
                {application.reminders.length} reminder{application.reminders.length !== 1 ? "s" : ""}
              </span>
            </div>
          )}
        ...
        {statusColumns.map((column) => {
          ...
          return (
            <div key={column.id} className="space-y-4">
              ...
              <div className="min-h-96 space-y-4" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, column.id)}>
                {columnApplications.map((application) => (
                  <ApplicationCard key={application.id} application={application} />
                ))}
