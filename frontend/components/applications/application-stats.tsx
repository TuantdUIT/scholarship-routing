...
export function ApplicationStats({ applications }: ApplicationStatsProps) {
  ...
  const completedApplications = applications.filter(
    (app) => app.status === "submitted" || app.status === "interview" || app.status === "result",
  ).length
  const inProgressApplications = applications.filter((app) => app.status === "in-progress").length
  const notStartedApplications = applications.filter((app) => app.status === "not-started").length

  const averageProgress =
    totalApplications > 0 ? Math.round(applications.reduce((sum, app) => sum + app.progress, 0) / totalApplications) : 0
  ...
  const totalReminders = applications.reduce((sum, app) => sum + app.reminders.length, 0)
