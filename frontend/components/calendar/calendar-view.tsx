...
export function CalendarView({
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push(<div key={`empty-${i}`} className="h-24 border border-muted" />)
    }
      ...
      {selectedDate && (
        <Dialog open={!!selectedDate} onOpenChange={() => setSelectedDate(null)}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Deadlines for {selectedDate.toLocaleDateString()}</DialogTitle>
              <DialogDescription>All deadlines and events scheduled for this date</DialogDescription>
            </DialogHeader>
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {getDeadlinesForDate(selectedDate).length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">No deadlines scheduled for this date</div>
              ) : (
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          <Badge className={`text-xs border-0 ${getPriorityColor(deadline.priority)}`}>
                            {deadline.priority}
                          </Badge>
                        </div>
                      </div>
