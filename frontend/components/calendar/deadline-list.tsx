...
import { Calendar, Clock, AlertTriangle, CheckCircle, ExternalLink, Edit, MoreHorizontal } from "lucide-react"
...
export function DeadlineList({ deadlines, onStatusChange }: DeadlineListProps) {
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case "critical":
        return <Badge className="bg-red-100 text-red-800 border-0">Critical</Badge>
      case "high":
        return <Badge className="bg-orange-100 text-orange-800 border-0">High</Badge>
      case "medium":
        return <Badge className="bg-blue-100 text-blue-800 border-0">Medium</Badge>
      case "low":
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "upcoming":
        return <Badge className="bg-orange-100 text-orange-800 border-0">Upcoming</Badge>
      case "pending":
        return <Badge variant="secondary">Pending</Badge>
      case "completed":
        return <Badge className="bg-green-100 text-green-800 border-0">Completed</Badge>
      case "waiting":
        return <Badge className="bg-purple-100 text-purple-800 border-0">Waiting</Badge>
      default:
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-muted-foreground">
              No deadlines match your current filters. Try adjusting your search criteria.
            </p>
          </CardContent>
        </Card>
      ) : (
        deadlines.map((deadline) => (
          <Card
            ...
            className={`hover:shadow-md transition-shadow ${
              deadline.daysLeft <= 7 ? "border-red-200" : deadline.daysLeft <= 14 ? "border-orange-200" : ""
            }`}
          >
            <CardContent className="p-6">
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-4 flex-1">
                  {/* Icon */}
                  <div className="flex-shrink-0 mt-1">{getDeadlineIcon(deadline.type)}</div>
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{deadline.provider}</p>
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm">
                      ...
                      {deadline.amount && <div className="text-muted-foreground">{deadline.amount}</div>}
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="w-full bg-muted rounded-full h-2">
                        <div
                          ...
                          style={{ width: `${getUrgencyProgress(deadline.daysLeft)}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" size="sm" asChild>
                          <Link href={`/scholarships/${deadline.scholarshipId}`}>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => onStatusChange(deadline.id, "completed")}>
                            Mark as Completed
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => onStatusChange(deadline.id, "pending")}>
                            Mark as Pending
                          </DropdownMenuItem>
